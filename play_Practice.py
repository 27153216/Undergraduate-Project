...
def resetGame():
    # 宣告使用全域變數.
    global game_mode, dx, dy, Collision
...
# 顯示Title.
pygame.display.set_caption(u"臉部表情辨識及其在老態預防上的應用--練習模式")
# 建立畫佈大小.
...
#底圖說明
titleRuleImg = pygame.image.load('image/rule4.png').convert()
titleRuleImg.set_alpha(100)
...
window = win32gui.FindWindow(None ,u'臉部表情辨識及其在老態預防上的應用--練習模式') #找到遊戲主視窗
