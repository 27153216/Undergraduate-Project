...
import os, sys, random, globals, threading, time, win32con, win32gui, win32api
import pygame 
...
def resetGame():
    # 宣告使用全域變數.
    global game_mode, brick_num, bricks_list, dx, dy, brick_quantity, Collision, Collision2
...
def detect():
    ...
    for i in indices:
        ...
        if emotion== "neutral":
            ...
            returnx = 10
        elif emotion == "happy" and game_mode == 0:
            ballSound.play()
            if dx < 0:
                dx = -dx
            game_mode = 1
            returnx = 0
        elif emotion == "surprised" and game_mode == 0:
            ballSound.play()
            if dx > 0:
                dx = -dx
            game_mode = 1
            returnx = 0
        break
    cv2.imshow("Facial Detection Down", image) #顯示畫面
    k = cv2.waitKey(1)
    if k == 27:
        running = False
...
def detect2():
    ...
    indices2 = cv2.dnn.NMSBoxes(boxes, confidences, conf_threshold, nms_threshold)
    returnx = 0
    for i in indices2:
        i = i[0]
        emotion = str(classes[class_ids[i]])
        ...
        if emotion== "neutral":
            ...
            returnx = 10
        elif emotion == "happy" and game_mode == 2:
            ballSound.play()
            if dx < 0:
                dx = -dx
            game_mode = 1
            returnx = 0
        elif emotion == "surprised" and game_mode == 2:
            ballSound.play()
            if dx > 0:
                dx = -dx
            game_mode = 1
            returnx = 0
        break
    cv2.imshow("Facial Detection Up", image) #顯示畫面
    cv2.waitKey(1)
    k = cv2.waitKey(1)
    if k == 27:
        running = False
...
# 顯示Title.
pygame.display.set_caption(u"臉部表情運動及老態預防--雙人對戰")
# 建立畫佈大小.
...
window = win32gui.FindWindow(None ,u'臉部表情運動及老態預防--雙人對戰') #找到遊戲主視窗
...
def movewindow():
    ...
    #    print(windowrect[0],windowrect[1],windowrect[2],windowrect[3]) #0:左上x 1:左上y 2右下x 3右下y
        #把攝影機畫面移到主視窗旁
        cv2.moveWindow("Facial Detection Down",windowrect[2],windowrect[1])
        cv2.moveWindow("Facial Detection Up",windowrect[0] - 660,windowrect[1])
        dps.tick(15)
